...
SELECT
    year AS Year, 
    SUM(electricity_from_renewables) AS TWH_from_renewables,  
    SUM(electricity_from_nuclear) AS TWH_from_nuclear,
    SUM(electricity_from_fossil_fuels) AS TWH_from_fossil_fuels, 
    SUM(electricity_from_renewables) + SUM(electricity_from_nuclear) AS TWH_from_renewables_and_nuclear,  
    SUM(electricity_from_renewables) + SUM(electricity_from_nuclear) + SUM(electricity_from_fossil_fuels) AS Total_TWH 
FROM 
    energy_data
WHERE
...
-- Step 1: Total global CO₂ emissions in 2023
